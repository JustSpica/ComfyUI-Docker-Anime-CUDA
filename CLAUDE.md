...
## Scope
- This repo is a Docker wrapper around upstream ComfyUI; ComfyUI source is cloned in `Dockerfile` at build time via `COMFYUI_REF` and is not vendored here.
- Runtime starts in `init_scripts/entrypoint.sh`: bootstrap extensions, bootstrap models, then run `python3 main.py`.
- `CLI_ARGS` from `docker-compose.yml` are appended only when the container runs the default `python3 main.py` command.

## Commands
- Check model URLs directly with `bash check_models_url.sh [models.conf]`; it loads `CIVITAI_API_KEY` from ignored `.env` if the variable is not already exported.
- Validate Compose config with `docker compose config --quiet`.
- Rebuild after Dockerfile or `init_scripts/*` changes with `docker compose up -d --build`; use `docker compose up -d` for normal startup.
- ComfyUI listens on `http://localhost:8188`.
- For a focused syntax check, run `bash -n init_scripts/<script>.sh` or `bash -n check_models_url.sh`.

## Runtime Data
- Treat `models/`, `input/`, `output/`, `custom_nodes/`, and `.env` as ignored runtime state, not source files to edit or commit.
- `workflows/` is tracked and bind-mounted to `/app/ComfyUI/user/default/workflows`; workflow JSON changes belong there.
- Extension Python packages live in Docker volume `comfyui_anime_venv`, not the image.
- To reset broken extension deps, run `docker compose down`, `docker volume rm comfyui_anime_venv`, remove `custom_nodes/.last_commits/*.commit`, then run `docker compose up -d --build`.

## Config Formats
- `models.conf` is the executable source of truth for model downloads; README model lists can lag behind it.
- Model sections map to lower-case ComfyUI folders, e.g. `[CHECKPOINTS]` to `models/checkpoints`, `[LORAS]` to `models/loras`, and `[UPSCALE_MODELS]` to `models/upscale_models`.
- Model entries support `URL|filename`, legacy `relative/path|URL`, or bare `URL`; `[CUSTOM]` and `[GIT_REPOS]` use `relative/path:https://...`.
- Model downloads skip any existing non-empty target file. Delete or rename the target file to force a re-download after changing a URL.
- `extensions.conf` supports `[EXTENSIONS]` or a plain URL list; each repo clones into `custom_nodes/<repo-basename>` and updates with `git pull --ff-only`.
- Extension deps from `requirements.txt` or `install.py` reinstall only when the repo commit differs from `custom_nodes/.last_commits/<name>.commit`; remove the marker to force reinstall.
- Config parsing strips anything after `#` or `;`; avoid those characters in active URL/config lines.

## Docker Notes
- Compose expects NVIDIA GPU access with `gpus: all` and exposes only port `8188`.
- The image runs as UID/GID `1000`; Dockerfile `COPY` lines for runtime scripts/configs should keep `--chown=1000:1000`.
- GPU packages are pinned to CUDA 12.8 builds: `torch==2.10.0+cu128`, `torchvision==0.25.0+cu128`, `torchaudio==2.10.0+cu128`, and `xformers==0.0.35`.
